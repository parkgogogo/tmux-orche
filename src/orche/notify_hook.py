...
EXPLICIT_CHANNEL_ID="${ORCHE_DISCORD_CHANNEL_ID:-}"
EXPLICIT_SESSION="${ORCHE_SESSION:-}"
payload=""

while [ $# -gt 0 ]; do
  case "$1" in
    --channel-id)
      EXPLICIT_CHANNEL_ID="${2:-}"
      shift 2
      ;;
    --session)
      EXPLICIT_SESSION="${2:-}"
...
channel_id="${EXPLICIT_CHANNEL_ID:-$(config_get "discord.channel-id")}"
[ -n "${channel_id}" ] || exit 0
