...
from session.meta import window_name
...
def _pane_record_from_tmux_output(output: str) -> Dict[str, str]:
    ...
    return {"session_name": parts[0], "pane_id": parts[1], "window_id": parts[2], "window_name": parts[3], "pane_dead": "0", "pane_pid": "", "pane_current_command": "", "pane_current_path": "", "pane_title": ""}
...
def create_dedicated_pane(session: str, cwd: Path) -> Dict[str, str]:
    ...
    result = tmux("new-session", "-d", "-s", tmux_name, "-n", window_name(session), "-c", str(cwd), "-P", "-F", _tmux_join_fields("#{session_name}", "#{pane_id}", "#{window_id}", "#{window_name}"), check=True, capture=True)
...
def _preferred_host_pane(*, tmux_session: str, host_pane_id: str = "", exclude_pane_id: str = "") -> str:
    ...
    for pane in list_panes(tmux_session):
        ...
        if pane_id and pane_id != exclude_pane_id and str(pane.get("pane_dead") or "") != "1":
            return pane_id
    raise RuntimeError(f"Unable to find a live host pane in tmux session: {tmux_session}")
...
def _create_temp_inline_pane(*, tmux_session: str, cwd: Path) -> Dict[str, str]:
    ...
    for _ in range(3):
        ...
        try:
            result = tmux("new-window", "-d", "-t", target, "-c", str(cwd), "-P", "-F", _tmux_join_fields("#{session_name}", "#{pane_id}", "#{window_id}", "#{window_name}"), check=True, capture=True)
            return _pane_record_from_tmux_output(result.stdout)
...
def _inline_group_sessions(*, tmux_session: str, host_pane_id: str, exclude_session: str = "") -> List[Dict[str, Any]]:
    ...
    for payload in _iter_meta_payloads():
        ...
        if host_session != tmux_session or not session_metadata_is_live(child_session, payload):
            continue
        ...
        if info is not None and str(info.get("session_name") or "").strip() == tmux_session:
            member = dict(payload)
...
def _normalize_inline_group_slots(group: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    def sort_key(payload: Mapping[str, Any]) -> Tuple[int, float, str]:
        ...
        return (DEFAULT_MAX_INLINE_SESSIONS if slot is None else slot, managed_session_last_event_at(payload, default=float("inf")), str(payload.get("session") or ""))
...
def _reflow_inline_panes(*, host_pane_id: str, pane_ids_by_slot: Mapping[int, str]) -> None:
    host_info = get_pane_info(host_pane_id)
    if host_info is None:
        raise RuntimeError(f"Unable to find host pane for inline layout: {host_pane_id}")
    ...
    slot_0, slot_1, slot_2, slot_3 = (pane_ids_by_slot.get(index, "") for index in range(4))
    pane_count = len({slot: pane for slot, pane in pane_ids_by_slot.items() if pane})
    if pane_count == 1 and slot_0:
        tmux("join-pane", "-d", "-h", "-l", "25%", "-s", slot_0, "-t", host_pane_id, check=True, capture=True); return
    if pane_count == 2 and slot_0 and slot_1:
        tmux("join-pane", "-d", "-h", "-l", "25%", "-s", slot_0, "-t", host_pane_id, check=True, capture=True)
        tmux("join-pane", "-d", "-v", "-l", "50%", "-s", slot_1, "-t", slot_0, check=True, capture=True); return
    if pane_count == 3 and slot_0 and slot_1 and slot_2:
        tmux("join-pane", "-d", "-h", "-l", "50%", "-s", slot_2, "-t", host_pane_id, check=True, capture=True)
        tmux("join-pane", "-d", "-h", "-l", "50%", "-s", slot_0, "-t", slot_2, check=True, capture=True)
        tmux("join-pane", "-d", "-v", "-l", "50%", "-s", slot_1, "-t", slot_0, check=True, capture=True); return
    if slot_0:
        tmux("join-pane", "-d", "-h", "-l", "50%", "-s", slot_0, "-t", host_pane_id, check=True, capture=True)
    if slot_1:
        tmux("join-pane", "-d", "-h", "-l", "50%", "-s", slot_1, "-t", slot_0, check=True, capture=True)
    if slot_2 and slot_0:
        tmux("join-pane", "-d", "-v", "-l", "50%", "-s", slot_2, "-t", slot_0, check=True, capture=True)
    if slot_3 and slot_1:
        tmux("join-pane", "-d", "-v", "-l", "50%", "-s", slot_3, "-t", slot_1, check=True, capture=True)


def create_inline_pane(session: str, cwd: Path, *, tmux_session: str, host_pane_id: str = "") -> Tuple[Dict[str, str], str]:
    resolved_host_pane = _preferred_host_pane(tmux_session=tmux_session, host_pane_id=host_pane_id)
    existing_group = _normalize_inline_group_slots(_inline_group_sessions(tmux_session=tmux_session, host_pane_id=resolved_host_pane, exclude_session=session))
    inline_limit = max_inline_sessions()
    if len(existing_group) >= inline_limit:
        raise RuntimeError(f"Inline pane limit reached for host pane {resolved_host_pane}: {inline_limit} session(s) max. Adjust inline.max-sessions (1-4) or close an existing inline session.")
    new_slot = len(existing_group)
    pane = _create_temp_inline_pane(tmux_session=tmux_session, cwd=cwd)
    try:
        pane_ids_by_slot = {int(member["inline_slot"]): str(member.get("pane_id") or "").strip() for member in existing_group if str(member.get("pane_id") or "").strip()}
        pane_ids_by_slot[new_slot] = pane["pane_id"]
        _reflow_inline_panes(host_pane_id=resolved_host_pane, pane_ids_by_slot=pane_ids_by_slot)
...
def ensure_pane(session: str, cwd: Path, agent: str, *, tmux_mode: str = "dedicated-session", host_pane_id: str = "", tmux_host_session: str = "") -> str:
    cwd = cwd.resolve()
    with session_lock(session):
        ...
        if pane_id and pane_exists(pane_id):
            ...
            if info is not None:
                ...
                meta.update({"backend": BACKEND, "session": session, "cwd": str(cwd), "agent": agent, "tmux_session": info["session_name"], "pane_id": pane_id, "window_id": info["window_id"], "window_name": info["window_name"], "tmux_mode": resolved_tmux_mode, "host_pane_id": resolved_host_pane_id, "tmux_host_session": resolved_tmux_host_session, "last_seen_at": time.time()})
        ...
        if resolved_tmux_mode == "inline-pane":
            ...
            with inline_host_lock(inline_tmux_session, inline_host_pane_id):
                pane, resolved_host_pane_id = create_inline_pane(session, cwd, tmux_session=inline_tmux_session, host_pane_id=inline_host_pane_id)
                pane_id = normalize_pane(session, cwd, pane)
                meta.update({"backend": BACKEND, "session": session, "cwd": str(cwd), "agent": agent, "tmux_session": pane["session_name"], "pane_id": pane_id, "window_id": pane["window_id"], "window_name": pane["window_name"], "tmux_mode": resolved_tmux_mode, "host_pane_id": resolved_host_pane_id, "tmux_host_session": resolved_tmux_host_session or pane["session_name"], "last_seen_at": time.time()})
...
def wait_for_agent_ready(plugin: AgentPlugin, pane_id: str, cwd: Path, *, timeout: float = STARTUP_TIMEOUT) -> str:
    ...
    while time.time() <= deadline:
        ...
        if any(prompt in capture for prompt in plugin.login_prompts):
            raise RuntimeError(f"{plugin.display_name} is not logged in inside the tmux pane")
        if str(sample.get("pane_dead") or "") == "1":
            raise RuntimeError(f"{plugin.display_name} pane exited before becoming ready: {pane_id}")
    ...
    raise RuntimeError(f"Timed out waiting for {plugin.display_name} to become ready in {pane_id}")


def wait_for_managed_startup_ready(session: str, plugin: AgentPlugin, pane_id: str, cwd: Path, *, timeout: float = STARTUP_TIMEOUT) -> str:
    deadline = time.time() + timeout
    ready_streak = 0
    while time.time() <= deadline:
        meta = load_meta(session)
        ...
        if startup_state == "blocked":
            raise AgentStartupBlockedError(str(startup.get("blocked_reason") or "").strip() or f"{plugin.display_name} startup blocked before reaching ready state in {pane_id}")
        if startup_state == "timeout":
            raise AgentStartupBlockedError(str(startup.get("blocked_reason") or "").strip() or f"{plugin.display_name} startup timed out before reaching ready state in {pane_id}")
        ...
        info = get_pane_info(pane_id)
        if info is None or info.get("pane_dead") == "1":
            raise RuntimeError(f"{plugin.display_name} pane exited before startup completed: {pane_id}")
        if not is_agent_running(plugin, pane_id):
            raise RuntimeError(f"{plugin.display_name} process exited before startup completed: {pane_id}")
        time.sleep(0.5)
...
def _managed_startup_reuse_wait_policy(session: str, plugin: AgentPlugin, pane_id: str, startup: Mapping[str, Any]) -> bool:
    startup_state = str(startup.get("state") or "").strip().lower()
    ...
    if startup_state in {"blocked", "timeout"}:
        ...
        raise RuntimeError(f"Session {session} is not ready because {detail}. Reopen or restart the session before sending prompts.")
    return False


def wait_for_agent_process_start(plugin: AgentPlugin, pane_id: str, *, timeout: float = STARTUP_TIMEOUT) -> str:
    deadline = time.time() + timeout
    last_capture = ""
    while time.time() <= deadline:
        capture = read_pane(pane_id, DEFAULT_CAPTURE_LINES)
        ...
        info = get_pane_info(pane_id)
        if info is None or info.get("pane_dead") == "1":
            raise RuntimeError(extract_launch_error(last_capture) or f"{plugin.display_name} pane exited before launch completed: {pane_id}")
        if is_agent_running(plugin, pane_id):
            return pane_id
        time.sleep(0.5)
    raise RuntimeError(extract_launch_error(last_capture) or f"Timed out waiting for {plugin.display_name} process to start in {pane_id}")


def ensure_agent_running(plugin: AgentPlugin, session: str, cwd: Path, pane_id: str, *, approve_all: bool = False, runtime: Optional[AgentRuntime] = None, discord_channel_id: Optional[str] = None) -> str:
    if is_agent_running(plugin, pane_id):
        return pane_id
    if get_pane_info(pane_id) is None:
        raise RuntimeError(f"Pane disappeared before {plugin.display_name} launch: {pane_id}")
    launch_command = plugin.build_launch_command(approve_all=True, cwd=cwd, runtime=runtime or AgentRuntime(label=plugin.runtime_label), session=session, discord_channel_id=discord_channel_id)
    tmux("respawn-pane", "-k", "-t", pane_id, "-c", str(cwd), launch_command, check=True, capture=True)
    pane_id = wait_for_agent_process_start(plugin, pane_id)
    bridge_name_pane(pane_id, session)
    meta = load_meta(session)
    meta.update({"backend": BACKEND, "session": session, "cwd": str(cwd), "pane_id": pane_id, "agent_started_at": time.time(), "agent_approve_all": True, "last_seen_at": time.time()})
...
def append_action_history(session: str, cwd: Path, agent: str, action: str, **fields: Any) -> None:
    append_history_entry(session, {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%S%z"), "backend": BACKEND, "session": session, "cwd": str(cwd), "agent": agent, "action": action, **fields})
    if action != "close":
...
def ensure_managed_claude_home(session: str, *, cwd: Path, discord_channel_id: Optional[str]) -> Path:
    return Path(prepare_managed_runtime(get_agent("claude"), session, cwd=cwd, discord_channel_id=discord_channel_id).home)
...
def build_native_agent_launch_command(plugin: AgentPlugin, *, session: str, cwd: Path, cli_args: Sequence[str]) -> str:
    ...
    prefix.append("if ! command -v " f"{shlex.quote(binary)} >/dev/null 2>&1; " f"then printf '%s\\n' {shlex.quote(launch_error)} >&2; sleep 2; exit 127; fi")
...
def ensure_native_agent_running(plugin: AgentPlugin, session: str, cwd: Path, pane_id: str, *, cli_args: Sequence[str]) -> str:
    if is_agent_running(plugin, pane_id):
        return pane_id
    if get_pane_info(pane_id) is None:
        raise RuntimeError(f"Pane disappeared before {plugin.display_name} launch: {pane_id}")
    launch_command = build_native_agent_launch_command(plugin, session=session, cwd=cwd, cli_args=cli_args)
    tmux("respawn-pane", "-k", "-t", pane_id, "-c", str(cwd), launch_command, check=True, capture=True)
    pane_id = wait_for_agent_process_start(plugin, pane_id)
    bridge_name_pane(pane_id, session)
    meta = load_meta(session)
    meta.update({"backend": BACKEND, "session": session, "cwd": str(cwd), "pane_id": pane_id, "agent_started_at": time.time(), "last_seen_at": time.time()})
    save_meta(session, meta)
    return pane_id
