...
def _pane_signature(*, tail: str, cursor_x: str, cursor_y: str, pane_in_mode: str, pane_current_command: str) -> str:
...
def sample_pane_state(plugin: AgentPlugin, pane_id: str, *, capture_lines: int = WATCHDOG_CAPTURE_LINES, agent_running_fn=None) -> Dict[str, Any]:
    ...
    return {
        ...
        "tail": tail,
        "signature": _pane_signature(tail=tail, cursor_x=str(cursor.get("cursor_x") or ""), cursor_y=str(cursor.get("cursor_y") or ""), pane_in_mode=str(cursor.get("pane_in_mode") or ""), pane_current_command=str((info or {}).get("pane_current_command") or "")),
        ...
        "agent_running": bool(resolved_pane_id and ((agent_running_fn(plugin, resolved_pane_id) if agent_running_fn is not None else _plugin_agent_running(plugin, resolved_pane_id)))),
    }
...
def observable_progress_detected(previous_signature: str, previous_cursor: tuple[str, str], sample: Mapping[str, Any]) -> bool:
    current_cursor = (str(sample.get("cursor_x") or ""), str(sample.get("cursor_y") or ""))
    return not previous_signature or previous_signature != str(sample.get("signature") or "") or previous_cursor != current_cursor or float(sample.get("cpu_percent") or 0.0) >= WATCHDOG_ACTIVE_CPU_THRESHOLD
